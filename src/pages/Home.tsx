...
export default function Home() {
  ...
  return (
    <main className="mx-auto flex min-h-svh max-w-4xl flex-col bg-canvas">
        ...
        {loading ? <><SkeletonCard /><SkeletonCard /></> : onboarding ? (
          <>
            <Card tone="highlight" className="relative overflow-hidden !p-6 sm:!p-8">
              <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-warn/15 text-warn"><UsersRound size={26} /></div>
              <h2 className="max-w-md font-serif text-3xl leading-tight sm:text-4xl">Meno messaggi.<br />Più tempo insieme.</h2>
              <p className="mb-7 mt-4 max-w-md text-base leading-relaxed text-fg-muted">Un unico posto per decidere dove andare, organizzare le auto e dividere le spese. Basta un invito.</p>
          ...
          <>
            ...
            {featured ? (
              ...
              <Card tone="highlight"><h2 className="font-serif text-xl">Il prossimo ritrovo inizia qui</h2><p className="mb-4 mt-2 text-sm text-fg-muted">Scegli un nome e invita gli amici. Potrai aggiungere luogo e data insieme a loro.</p><Button onClick={() => openSheet('event')}><Plus size={18} /> Crea un evento</Button></Card>
            )}
      ...
      {!loading && !onboarding && <div className="bottom-actions fixed inset-x-0 bottom-0 z-20 mx-auto flex max-w-lg gap-3 bg-gradient-to-t from-canvas via-canvas px-4 pt-4"><Button variant="surface" className="flex-1" onClick={() => openSheet('join')}><Ticket size={17} /> Invito</Button><Button className="flex-1" onClick={() => openSheet('event')}><Plus size={17} /> Nuovo evento</Button></div>}
...
function EventCard({ summary: { room, memberCount }, featured = false, onOpen }: { summary: RoomSummary; featured?: boolean; onOpen: () => void }) {
  return <Card tone={featured ? 'highlight' : 'surface'} onClick={onOpen} className={featured ? '!p-6 sm:!p-8' : ''}>
    <div className="mb-4 flex items-center justify-between gap-3"><Chip tone={room.status === 'closed' ? 'muted' : 'teal'}>{room.status === 'closed' ? 'Archiviato' : 'In programma'}</Chip><span className="flex items-center gap-1.5 text-sm text-fg-muted"><UsersRound size={15} />{memberCount}</span></div>
    <h3 className={`break-words font-serif ${featured ? 'text-3xl' : 'text-lg'}`}>{room.title}</h3>
    <div className="mt-4 space-y-2 text-sm text-fg-muted"><p className="flex items-start gap-2"><Calendar size={17} className="mt-0.5 shrink-0" />{room.event_time ? formatEventTime(room.event_time) : 'Data da scegliere'}</p><p className="flex items-start gap-2"><MapPin size={17} className="mt-0.5 shrink-0" />{room.destination_label || 'Destinazione da scegliere'}</p></div>
    <div className={`mt-5 flex items-center justify-between border-t border-line pt-4 text-sm font-medium ${featured ? 'text-warn' : 'text-fg'}`}><span>{room.status === 'closed' ? 'Consulta il riepilogo' : 'Apri evento'}</span><ArrowUpRight size={19} /></div>
  </Card>
}

...
export default function PersonalSummary(props: Props) {
  ...
  return <Card tone="highlight" className="!p-5 sm:!p-6">
    <p className="mb-3 text-xs font-medium uppercase tracking-widest text-warn">Il tuo prossimo passo</p>
    <h2 className="font-serif text-2xl leading-tight">{title}</h2>
    <p className="mb-5 mt-2 text-sm leading-relaxed text-fg-muted">{dataIncomplete ? 'Alcuni dati non sono disponibili. Ricarica l’evento prima di organizzare posti e conti.' : needsConfirmation ? 'Conferma la presenza: aiuti tutti a organizzare posti e partenza.' : myCar ? 'Il tuo posto è organizzato. Trovi passeggeri, carico e aggiornamenti nella sezione Auto.' : 'Scegli un posto disponibile oppure metti a disposizione la tua auto.'}</p>
    <div className="mb-5 grid grid-cols-2 gap-3 border-y border-line py-4">
      <div><p className="flex items-center gap-1.5 text-xs text-fg-muted"><CarIcon size={14} /> Il tuo passaggio</p><p className="mt-1 text-sm font-medium">{dataIncomplete ? 'Da verificare' : myCar ? myCar.driver_member_id === currentMember.id ? 'Sei alla guida' : `Auto di ${driver?.display_name ?? 'un amico'}` : 'Da organizzare'}</p></div>
      <div><p className="flex items-center gap-1.5 text-xs text-fg-muted"><Receipt size={14} /> Il tuo saldo</p><p className="mt-1 text-sm font-medium">{balance === null ? 'Non disponibile' : balance > 0 ? `Ricevi ${formatMoney(balance)}` : balance < 0 ? `Devi ${formatMoney(-balance)}` : 'Nessun saldo aperto'}</p></div>
    </div>
